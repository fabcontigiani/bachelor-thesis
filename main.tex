...
Durante las pruebas de campo, se observó que la red mantuvo conectividad operativa de forma estable siempre que se cumplieran dos condiciones: que los nodos dispusieran de carga de batería suficiente y que la calidad de la señal Wi-Fi entre nodos adyacentes fuera adecuada (RSSI superior a \qty{-75}{dBm}). Los principales factores que incidieron en la estabilidad fueron:

\begin{itemize}
  \item \textbf{Calidad de señal entre nodos:} En condiciones de buena señal, las desconexiones espontáneas resultaron poco frecuentes. La vegetación densa y las condiciones climáticas adversas degradaron la señal en algunos casos, provocando reconexiones esporádicas.
  \item \textbf{Agotamiento de batería:} Cuando la tensión del pack descendía por debajo del umbral funcional del regulador, el nodo se apagaba de forma definitiva y no se reincorporaba a la red hasta ser recargado manualmente.
  \item \textbf{Saturación del punto de acceso:} En escenarios con múltiples dispositivos conectados al mismo router Wi-Fi, se observaron retardos adicionales en la transmisión de imágenes, aunque sin pérdida total de conectividad.
  \item \textbf{Profundidad de la topología:} A mayor cantidad de saltos en la red de árbol, mayor la probabilidad de que una desconexión en un nodo intermedio afecte a los nodos descendientes.
\end{itemize}
