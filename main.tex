...
Se verificó la transmisión de imágenes a través de la red \gls{mesh}, comprobando el establecimiento inicial de la conexión entre nodos y la transferencia exitosa de las cargas útiles completas. Adicionalmente, se evaluó el tiempo de transmisión desde el nodo origen hasta el servidor y se observó el comportamiento del mecanismo de recuperación ante pérdidas de conectividad temporales.
...
Cabe destacar que, para el desarrollo de las pruebas de campo, el nodo raíz se vinculó a internet mediante un punto de acceso móvil (\textit{hotspot}) provisto por un teléfono celular, en lugar de utilizar un enrutador comercial dedicado. Si bien esta decisión logística permitió validar el despliegue del sistema en un entorno no urbanizado y carente de infraestructura, la variabilidad propia de las redes de telefonía móvil introdujo un grado de inestabilidad que impactó directamente en los tiempos de respuesta. Un enrutador fijo provisto de antenas de mayor ganancia ofrecería un canal ascendente más holgado, lo cual redundaría en una mejora sobre los tiempos observados.
...
Los resultados consolidados de estas observaciones se exponen en la Tabla~\ref{tab:distancia}. Se evidencia que la antena integrada en la placa de circuito impreso (PCB) del módulo ESP32-CAM presenta una sensibilidad limitada, siendo altamente susceptible a la atenuación provocada por obstáculos físicos. En entornos caracterizados por vegetación densa, el rango útil experimentó reducciones considerables en comparación con el escenario ideal de línea de vista directa, un factor crítico a tener en cuenta para la planificación del espaciado entre nodos durante un despliegue real.
...
\begin{itemize}
  ...
  \item El alcance efectivo de la antena PCB integrada se redujo considerablemente en presencia de vegetación, requiriendo distancias entre nodos inferiores a las teóricamente posibles.
